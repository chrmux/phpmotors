...
function getImages() {
    $db = phpmotorsConnect();
    $sql = 'SELECT imgId, imgPath, imgName, imgDate, inventory.invId, invMake, invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.imgPath LIKE "%tn%"';
    $stmt = $db->prepare($sql);
    $stmt->execute();
